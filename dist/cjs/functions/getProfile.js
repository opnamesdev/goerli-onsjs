  ...
  if (specificResolver) {
    ...
    try {
      const resolvedData = await Promise.all(
        data.map(
          (d) => universalResolver["resolve(bytes,bytes)"]((0, import_hexEncodedName.hexEncodeName)(name), d, {
            ccipReadEnabled: true
          })
        )
      );
      recordData = resolvedData.map((r) => r[0]);
      const [_, _resolver] = resolvedData[0];
      ...
      console.log("getDataForName", "err", err);
